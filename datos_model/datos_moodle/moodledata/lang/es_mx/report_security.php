...
/**
 * Strings for component 'report_security', language 'es_mx', branch 'MOODLE_31_STABLE'
 *
...
$string['check_nodemodules_details'] = '<p>El directorio <em>{$a->path}</em> contiene módulos Node.js y sus dependencias, típicamente instalados por la utilería NPM. Estos módulos podrían se rnecesarios para el desarrollo Moodle. No son necesarios para correr un sitio Moodle y podrían contener código peligroso que exponga su sitio a ataques remotos.</p><p>Se recomienda encarecidamente que quite el directorio si el sitio está disponible vía una URL pública, o cuando menos que prohiba el acceso web a él.</p>';
...
$string['check_vendordir_details'] = '<p>El directorio vendor (del vendedor) <em>{$a->path}</em> contiene varias librerías de terceros y sus dependencias, típicamente instaladas por el PHP Composer.Podría ser necesario para el desarrollo local, como por ejemplo para instalar el PHPUnit framework. Pero también podría contener código potencialmente peligroso, que expondría su sitio a ataques remotos.</p><p>Se recomienda encarecidamente que quite el directorio si el sitio está disponible vía una URL pública, o cuando menos que prohíba el acceso web a él.</p>';
